...
void* serial_wait(void* serial_ptr)
{
  ...
  printf("GG Starting serial_wait");
  ...
  while (1)
  {
    ...
    {
      if (!silent)
        //fprintf(stderr, ".."); //GG changed so it didn't fill terminal while USB not working
 ...
 * and forwards the data once received.
 */
void* gg_cli(void* serial_ptr)
{
 fprintf(stderr, "GG Thread Started. Enter Command:\n");
}

void quad_start(void)
{
  fprintf(stderr, "GG Quad Start.\n");
...
int main(int argc, char **argv)
{
  ...
  if (fd == -1)
  {
    ...
    fprintf(stderr, "GG Port fail error: %d.\n", errno);
    exit(EXIT_FAILURE);
  }
  else
  {
  ...
  if ((gg_cli_thread = g_thread_create((GThreadFunc)gg_cli, (void *)fd_ptr, TRUE, &err)) == NULL)
  {
    printf("Failed to create gg_cli thread: %s!!\n", err->message);
    g_error_free(err);
  }
